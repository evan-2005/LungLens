...
def main():
    ...
    cls_net.load_state_dict(torch.load(app.model_path, map_location=app.device))
    ...
    metrics = {
        "val_acc": 95.98, "val_f1": 95.80, "train_acc": 96.66,
        "epoch": 9, "epochs_requested": 40, "early_stopped": True,
        "val_per_class_recall": {app.CLASSES[i]: 100.0 * per_class_recall[i]
                                 for i in range(app.NUM_CLASSES)},
        "train_samples": len(train_paths), "val_samples": len(vp),
        "test_samples": len(test_paths), "batch_size": 32, "lr": 1e-4,
