...
\node[artefact, proc, text width=17mm, right=24mm of s2-teach] (s2-cam) {Cached CAM target\\{\tiny ReLU, resize $224^2$,\\min--max norm.\\precomputed once}};
\node[data, proc, text width=15mm, below=9mm of s2-teach] (s2-img) {\tiny Preprocessed image};
\node[train, proc, text width=15mm, right=24mm of s2-img] (s2-unet) {\textbf{U-Net}\\\textbf{student} $\theta_s$};
\node[loss, minimum width=13mm, minimum height=12mm] (s2-loss) at ($(s2-cam.east)!0.5!(s2-unet.east)+(11mm,0)$) {\tiny Dice\\+ BCE};

\draw[fwd] (s2-teach) -- node[lbl, above, text width=17mm, pos=0.5]{Grad-CAM @\\\texttt{\scriptsize denseblock4.}\\\texttt{\scriptsize denselayer16.conv2}} (s2-cam);
...
\node[tag, above=0.5mm of s3-rep] (tagS) {S0--S4 enters here};
\draw[okvermillion, line width=0.45pt, -{Latex[length=1mm]}] (tagS.south) -- (s3-rep.north);
...
\draw[fwd] (s3-gate1.east) -| node[lbl, pos=0.62, above, text width=16mm]{label + confidence} (s3-rep.north);
